...
class Bwa:
    def __init__(self,version="default", samtools_version="default"):
        try:
            ...
            print("Cannot find specified version of bwa in configuration file: %s" % version, file=sys.stderr)
            sys.exit(1)
        try:   
            self.samtools_cmd=util.programs["samtools"][samtools_version]
        except KeyError as e:
            print("Cannot find specified version of samtools in configuration file: %s" % version, file=sys.stderr)
            sys.exit(1)
    def index(self, fasta, args_dict=None):
        if not os.path.isfile(fasta):
            print("You must supply a fasta file to bwa index: %s is not a file" % fasta, file=sys.stderr)
            sys.exit(1)
